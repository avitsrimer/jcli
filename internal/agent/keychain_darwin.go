...
// item lands in the default (login) keychain and the creating binary's ad-hoc code identity is added
// to its ACL. Returns the OSStatus (errSecSuccess on success).
...
// getItem reads the token for service/account with a plain SecItemCopyMatching. The creating binary's
// ad-hoc code identity is trusted by the item's ACL and reads it back silently; a binary with a
// different code identity (e.g. after a rebuild, since the ad-hoc cdhash changes) triggers the
// standard keychain "Allow / Always Allow" prompt. On success it returns errSecSuccess and sets
// *outData / *outLen to a malloc'd buffer the caller must free.
...
// prompt that a binary with a different code identity triggers, so it deliberately reads as the
// product name.
...
// item has no access-control, so ACL trust — not an entitlement — gates reads). The constant and its
// hint remain only as a safety net.
...
// item in the default (login) keychain; the item's trusted-application ACL is bound to the ad-hoc
// code identity of the binary that created it, so that same binary reads it back without a prompt. It
// is not safe for concurrent use; the agent serializes access through its request loop.
...
// Get reads the token for profile. The ad-hoc code identity of the binary that created the item is
// trusted by its ACL and reads it back silently; a binary with a different code identity hits the
// standard keychain "Allow / Always Allow" prompt.
